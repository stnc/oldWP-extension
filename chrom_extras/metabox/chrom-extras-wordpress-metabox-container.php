...
function CHfw__mpevent_conditions_selected_html($post)
{
	...
	?>
    <select name="CHfw_DrAndDep_proAndServices_SubConditions[]" multiple id="CHfw_DrAndDep_proAndServices_SubConditions">
...
function CHfw_mpevent_conditions_selected_save($post_id)
{
	...
	if (isset($_POST['CHfw_DrAndDep_proAndServices_SubConditions'])) {
		foreach ($_POST['CHfw_DrAndDep_proAndServices_SubConditions'] as $selectedOption) {
			$selectedOptionlist[] = $selectedOption;
		}
		$selectedOptionlist = implode(",", $selectedOptionlist);
		update_post_meta($post_id, 'CHfw_DrAndDep_proAndServices_SubConditions', esc_attr($selectedOptionlist));
	}
